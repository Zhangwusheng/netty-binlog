...
//        this(new ByteBuf(bytes));
        this(Unpooled.copiedBuffer(bytes));
    }

    public JsonBinary(ByteBuf contents) {
        ...
        for (int i = 0; i != numElements; ++i) {
            ...
            switch (type) {
                case LITERAL:
                    ...
                    reader.skipBytes(valueSize - 1);
                    break;
                case INT16:
                    entries[i] = new ValueEntry(type).setValue(readInt16());
                    reader.skipBytes(valueSize - 2);
                    break;
                case UINT16:
...
//            keys[i] = reader.readString(keyLengths[i]);
            keys[i] = ByteUtil.readString(reader,keyLengths[i]);
        }
...
//        String value = new String(reader.read(length), UTF_8);
        String value = ByteUtil.readString(reader,length);
        formatter.value(value);
    }
...
//        int customType = reader.read();
        int customType = reader.readByte();
...
//        BigDecimal dec = AbstractRowsEventDataDeserializer.asBigDecimal(precision, scale, reader.read(decimalLength));
        BigDecimal dec = AbstractRowsEventDataDeserializer.asBigDecimal(precision, scale, ByteUtil.readSpecifiedLengthBytes(reader,decimalLength));
        formatter.value(dec);
    }
...
//        formatter.valueOpaque(type, reader.read(length));
        formatter.valueOpaque(type, ByteUtil.readSpecifiedLengthBytes(reader,length));
    }
...
//        byte[] bytes = reader.read(numBytes);
        byte[] bytes = ByteUtil.readSpecifiedLengthBytes(reader,numBytes);
        long result = 0;
    ...
    protected long readInt64() throws IOException {
        ...
        int b2 = reader.readByte() & 0xFF;
        int b3 = reader.readByte() & 0xFF;
        long b4 = reader.readByte() & 0xFF;
        long b5 = reader.readByte() & 0xFF;
        long b6 = reader.readByte() & 0xFF;
        long b7 = reader.readByte() & 0xFF;
        long b8 = reader.readByte();
    ...
    protected BigInteger readUInt64() throws IOException {
        ...
        for (int i = 8; i != 0; --i) {
            bigEndian[i - 1] = (byte) (reader.readByte() & 0xFF);
        }
