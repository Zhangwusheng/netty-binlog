...
public abstract class AbstractRowsEventDataDeserializer<T extends IEventData > implements EventDataDeserializer<T> {
    ...
    protected Serializable[] deserializeRow(long tableId, BitSet includedColumns, ByteBuf inputStream) {
...
//        BitSet nullColumns = inputStream.readBitSet(result.length, true);
        BitSet nullColumns = ByteUtil.readBitSet(inputStream,result.length, true);
    ...
    protected Serializable deserializeCell(ColumnType type, int meta, int length, ByteBuf inputStream)
             {
        switch (type) {
            case BIT:
...
//                throw new IOException("Unsupported type " + type);
    ...
    protected Serializable deserializeBit(int meta, ByteBuf inputStream) {
        int bitSetLength = (meta >> 8) * 8 + (meta & 0xFF);
        //return inputStream.readBitSet(bitSetLength, false);
        return ByteUtil.readBitSet(inputStream,bitSetLength, false);
    }

    protected Serializable deserializeTiny(ByteBuf inputStream)  {
//        return (int) ((byte) inputStream.readInteger(1));
        return (int) ((byte) ByteUtil.readInteger(inputStream,1));
    }

    protected Serializable deserializeShort(ByteBuf inputStream)  {
//        return (int) ((short) inputStream.readInteger(2));
        return (int) ((short) ByteUtil.readInteger(inputStream,2));
    }

    protected Serializable deserializeInt24(ByteBuf inputStream)  {
//        return (inputStream.readInteger(3) << 8) >> 8;
        return (ByteUtil.readInteger(inputStream,3) << 8) >> 8;
    }

    protected Serializable deserializeLong(ByteBuf inputStream)  {
//        return inputStream.readInteger(4);
        return ByteUtil.readInteger(inputStream,4);
    }

    protected Serializable deserializeLongLong(ByteBuf inputStream) {
//        return inputStream.readLong(8);
        return ByteUtil.readUnsignedLong(inputStream,8);
    }

    protected Serializable deserializeFloat(ByteBuf inputStream)  {
//        return Float.intBitsToFloat(inputStream.readInteger(4));
        return Float.intBitsToFloat(ByteUtil.readInteger(inputStream,4));
    }

    protected Serializable deserializeDouble(ByteBuf inputStream)  {
//        return Double.longBitsToDouble(inputStream.readLong(8));
        return Double.longBitsToDouble(ByteUtil.readUnsignedLong(inputStream,8));
    }

    protected Serializable deserializeNewDecimal(int meta, ByteBuf inputStream) {
...
//        return asBigDecimal(precision, scale, inputStream.read(decimalLength));
        return asBigDecimal(precision, scale, ByteUtil.readSpecifiedLengthBytes(inputStream,decimalLength));
    }
    ...
    protected Serializable deserializeDate(ByteBuf inputStream)  {
//        int value = inputStream.readInteger(3);
        int value = ByteUtil.readInteger(inputStream,3);
        int day = value % 32;
        value >>>= 5;
        int month = value % 16;
        int year = value >> 4;
    ...
    protected Serializable deserializeTime(ByteBuf inputStream) {
//        int value = inputStream.readInteger(3);
        int value = ByteUtil.readInteger(inputStream,3);
    ...
    protected Serializable deserializeTimeV2(int meta, ByteBuf inputStream)  {
        /*
            (in big endian)
...
//        long time = bigEndianLong(inputStream.read(3), 0, 3);
        long time = bigEndianLong(ByteUtil.readSpecifiedLengthBytes(inputStream,3), 0, 3);
    ...
    protected Serializable deserializeTimestamp(ByteBuf inputStream)  {
//        long timestamp = inputStream.readLong(4) * 1000;
        long timestamp = ByteUtil.readUnsignedLong(inputStream,4) * 1000;
    ...
    protected Serializable deserializeTimestampV2(int meta, ByteBuf inputStream) {
//        long millis = bigEndianLong(inputStream.read(4), 0, 4);
        long millis = bigEndianLong(ByteUtil.readSpecifiedLengthBytes(inputStream,4), 0, 4);
        int fsp = deserializeFractionalSeconds(meta, inputStream);
    ...
    protected Serializable deserializeDatetime(ByteBuf inputStream)  {
//        int[] split = split(inputStream.readLong(8), 100, 6);
        int[] split = split(ByteUtil.readUnsignedLong(inputStream,8), 100, 6);
    ...
    protected Serializable deserializeDatetimeV2(int meta, ByteBuf inputStream)  {
        /*
            (in big endian)
...
//        long datetime = bigEndianLong(inputStream.read(5), 0, 5);
        long datetime = bigEndianLong(ByteUtil.readSpecifiedLengthBytes(inputStream,5), 0, 5);
    ...
    protected Serializable deserializeYear(ByteBuf inputStream)  {
//        return 1900 + inputStream.readInteger(1);
        return 1900 + ByteUtil.readInteger(inputStream,1);
    }

    protected Serializable deserializeString(int length, ByteBuf inputStream)  {
        ...
        int stringLength = length < 256 ?
//                inputStream.readInteger(1)
        ByteUtil.readInteger(inputStream,1)
//                : inputStream.readInteger(2);
        :ByteUtil.readInteger(inputStream,2);
        if (deserializeCharAndBinaryAsByteArray) {
//            return inputStream.read(stringLength);
            return ByteUtil.readSpecifiedLengthBytes(inputStream,stringLength);
        }

        byte[] bytes =  ByteUtil.readSpecifiedLengthBytes(inputStream,stringLength);
        return new String(bytes);
    }

    protected Serializable deserializeVarString(int meta, ByteBuf inputStream)  {
//        int varcharLength = meta < 256 ? inputStream.readInteger(1) : inputStream.readInteger(2);
        int varcharLength = meta < 256 ?
                ByteUtil.readInteger(inputStream,1)
                :ByteUtil.readInteger(inputStream,2);
        if (deserializeCharAndBinaryAsByteArray) {
//            return inputStream.read(varcharLength);
            return ByteUtil.readSpecifiedLengthBytes(inputStream,varcharLength);
        }
//        return inputStream.readString(varcharLength);
        byte[] bytes =  ByteUtil.readSpecifiedLengthBytes(inputStream,varcharLength);
        return new String(bytes);
    }

    protected Serializable deserializeBlob(int meta, ByteBuf inputStream) {
//        int blobLength = inputStream.readInteger(meta);
        int blobLength = ByteUtil.readInteger(inputStream,meta);
//        return inputStream.read(blobLength);
        return ByteUtil.readSpecifiedLengthBytes(inputStream,blobLength);
    }

    protected Serializable deserializeEnum(int length, ByteBuf inputStream) {
//        return inputStream.readInteger(length);
        return ByteUtil.readInteger(inputStream,length);
    }

    protected Serializable deserializeSet(int length, ByteBuf inputStream)  {
//        return inputStream.readLong(length);
        return ByteUtil.readUnsignedLong(inputStream,length);
    }

    protected Serializable deserializeGeometry(int meta, ByteBuf inputStream)  {
//        int dataLength = inputStream.readInteger(meta);
        int dataLength = ByteUtil.readInteger(inputStream,meta);
//        return inputStream.read(dataLength);
        return ByteUtil.readSpecifiedLengthBytes(inputStream,dataLength);
    }
     ...
     */
    protected byte[] deserializeJson(int meta, ByteBuf inputStream)  {
//        int blobLength = inputStream.readInteger(meta);
//        return inputStream.read(blobLength);
        int blobLength = ByteUtil.readInteger(inputStream,meta);
        return ByteUtil.readSpecifiedLengthBytes(inputStream,blobLength);
    ...
    protected int deserializeFractionalSeconds(int meta, ByteBuf inputStream)  {
        int length = (meta + 1) / 2;
        if (length > 0) {
//            int fraction = bigEndianInteger(inputStream.read(length), 0, length);
            int fraction = bigEndianInteger(ByteUtil.readSpecifiedLengthBytes(inputStream,length), 0, length);
