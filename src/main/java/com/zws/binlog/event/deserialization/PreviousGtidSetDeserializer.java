...
import com.zws.binlog.event.data.PreviousGtidSetEventData;
...
public class PreviousGtidSetDeserializer implements EventDataDeserializer<PreviousGtidSetEventData> {
    ...
    public PreviousGtidSetEventData deserialize(ByteBuf inputStream)  {
//        int nUuids = inputStream.readInteger(8);
        int nUuids = ByteUtil.readInteger(inputStream,8);
        String[] gtids = new String[nUuids];
        for (int i = 0; i < nUuids; i++) {
            byte[] byteUuid = new byte[16];
            inputStream.readBytes(byteUuid);
            String uuid = formatUUID(byteUuid);

            int nIntervals = ByteUtil.readInteger(inputStream,8);
            String[] intervals = new String[nIntervals];
            for (int j = 0; j < nIntervals; j++) {
                long start = ByteUtil.readUnsignedLong(inputStream,8);
                long end = ByteUtil.readUnsignedLong(inputStream,8);
